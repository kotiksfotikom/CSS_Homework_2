...
Original file is located at
    https://colab.research.google.com/drive/1rylhtUkG37hvjBu6Xgza39Bj0KvPx4Fb
...
def concatenate(seqs):
    return [i for seq in seqs for i in seq]
...
def transpose(matrix):
    new_rows_len = len(matrix)
    new_cols_len = len(matrix[0])
    new_matrix = []

    for i in range(new_cols_len):
        new_row = []
        for j in range(new_rows_len):
...
def digits_to_words(text):
    ...
    return ' '.join(digits_words[int(char)] for char in text if char.isdigit())
...
def to_mixed_case(name):
    ...
    for char in name:
        if char == '_':
            ...
            result += char.upper() if cap_next else char.lower()
            cap_next = False
...
def format_date(date):
    return datetime.strptime(date, "%Y-%m-%d %H:%M:%S").timestamp()/60
...
def get_work_time(df):
    work_times = df['SubmitTime'] - df['AcceptTime']
    return df.groupby('WorkerId')[['SubmitTime', 'AcceptTime']].apply(
        lambda x: (x['SubmitTime'] - x['AcceptTime']).sum()
    )
...
def calculated_work_time(df):
    ...
    for worker in df['WorkerId'].unique():
        worker_hits = df[df['WorkerId'] == worker].copy()
        worker_hits = worker_hits.sort_values('AcceptTime')

        working_periods = []
        for idx, hit in worker_hits.iterrows():
            start_time = hit['AcceptTime']
            end_time = hit['SubmitTime']
            working_periods.append([start_time, end_time])

        if len(working_periods) == 0:
            worker_times[worker] = 0
            continue

        working_periods.sort()
        merged_periods = [working_periods[0]]

        for period in working_periods[1:]:
            last_period = merged_periods[-1]

            if period[0] <= last_period[1]:
                last_period[1] = max(last_period[1], period[1])
            else:
                merged_periods.append(period)

        total_work_time = 0
        for period_start, period_end in merged_periods:
            total_work_time += (period_end - period_start)

        worker_times[worker] = total_work_time
...
def extract_revenue(df, column, name):
    ...
    def parse_money(money_str):
        try:
            amount = price_dec(str(money_str))
            return round(amount / 1_000_000, 2) if amount else 0
        except:
            return 0

    result_df[name] = result_df[column].apply(parse_money)
    return result_df
